...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        """
        newset=set()
        for nodelit in self.all_actions:
            pgNodea=PgNode_a(nodelit)
            #print(pgNodea.prenodes)
            if pgNodea.prenodes.issubset(self.s_levels[level]): # selecting actions that had preconditions
                #print('yes')
                for pren in pgNodea.prenodes:
                    for nodess in self.s_levels[level]:
                        if (pren.__eq__(nodess)):
                            pgNodea.parents.add(nodess)
                #print(pgNodea.parents)

                newset.add(pgNodea)  # adding  to a set for level a
        self.a_levels.append(newset)
            #for s in self.s_levels[level]:        
            #pass #print(s.show(), s.parents, self.problem.actions_list)
        #print(self.noop_actions())
    ...
    def add_literal_level(self, level):
        ...
        for act in self.a_levels[level-1]:
            pgNodes_set=act.effnodes # potential children as PgNode_s
            while len(pgNodes_set):
                pgNode=pgNodes_set.pop()
                if pgNode.symbol: 
                    pgNode.parents.add(act)
                    act.children.add(pgNode)
                    newset.add(pgNode)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        for p1 in node_a1.action.effect_add:
            for p2 in node_a2.action.effect_rem:
                if p1==p2: return True 
        for p1 in node_a1.action.effect_rem:
            for p2 in node_a2.action.effect_add:
                if p1==p2: return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #    for p2 in node_a2.action.precond_neg:
        #        if p1==p2: return True 
        #for p1 in node_a2.action.precond_pos:
        #    for p2 in node_a1.action.precond_neg:
        #        if p1==p2: return True
        #return False        
        # TODO test for Competing Needs between nodes
        for p2 in node_a2.parents:  # these 5 lines also deliver working function
            for p1 in node_a1.parents:
                if not p1.is_mutex(p2): 
                    if not p2.is_mutex(p1): return False # if there is a pair of parents that are not exlusive returns False
        return True

        #first_set = set(node_a2.action.precond_neg) 
        #if any(True for expr in node_a1.action.precond_pos if expr in first_set): return True
        #first_set = set(node_a1.action.precond_neg) 
        #if any(True for expr in node_a2.action.precond_pos if expr in first_set): return True
        #return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        return (isinstance(node_s1, node_s2.__class__) and node_s1.is_pos!=node_s2.is_pos and node_s1.symbol==node_s2.symbol)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        #mutex=(node_s2 in node_s1.mutex or node_s1 in node_s2.mutex) 
        #if mutex: return True
        #for p1 in node_s1.parents:
        #    for p2 in node_s2.parents:
        #        if not (self.competing_needs_mutex(p1, p2) or self.inconsistent_effects_mutex(p1,p2) or self.interference_mutex(p1,p2)): 
        #            return False
        #return True 

        for p2 in node_s2.parents:  # these 5 lines also deliver working function
            for p1 in node_s1.parents:
                if not p1.is_mutex(p2): 
                    if not p2.is_mutex(p1): return False # if there is a pair of parents that are not exlusive returns False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        :return: int
        """
        goals=set(self.problem.goal) # putting all goals into set, where each goal will be poped out cycling through the set
        level_sum = 0
        while len(goals):
            goal=goals.pop()
            lstep=0
            for level in self.s_levels:
                for node in level:
                    is_that_level=goal == node.symbol
                    if is_that_level and node.is_pos: 
                        level_sum+=lstep
                        break #found that goal break out from inner loop
                if is_that_level and node.is_pos: break    #break out from the second inner loop
                lstep+=1
        return level_sum
